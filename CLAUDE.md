...
- FOR ANY BUG, TEST FAILURE, OR UNEXPECTED BEHAVIOR, YOU MUST follow the systematic debugging framework. See the **superpowers:systematic-debugging** skill for complete methodology.
...
- Your context window is automatically compacted as it approaches its limit. Do not stop tasks early due to token budget concerns. Save progress to memory before context refreshes.
- At conversation start, review the recent context index in CLAUDE.md before diving into tasks
- YOU MUST search the journal/memory system before asking Mark to repeat information
