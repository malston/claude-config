...
- **Global scope**: `/Users/markalston/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/markalston/.claude/notes/`)
...
2. Glob `/Users/markalston/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/markalston/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/markalston/.claude/notes/` — works in any directory
